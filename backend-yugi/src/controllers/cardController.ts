...
export const getCards = async (req: Request, res: Response) => {
    ...
    const { fname, type, attribute, race, level, atk, offset, num } = validatedQuery;

    const banlistStatusMap = await fetchBanlistStatus();

    const apiParams: any = { fname, type, attribute, race, level, atk, offset, num };
...
function getATKValue(atk: number | string | null | undefined): number {
    if (atk === null || atk === undefined) return 0;
    if (typeof atk === 'number') return atk;
    
    const atkStr = String(atk).toUpperCase();
    if (atkStr === 'X') return 9999; 
    if (atkStr === '?') return 0; 

    const parsed = parseInt(atkStr, 10);
